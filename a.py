...
#set page config
st.set_page_config(page_title="HomeWatt",
                   page_icon="💡",
                   layout="wide")
...
similaritty_model=sentenceTransformers('all-MiniLM-L6-v2')
recommendations=[
    "use energy efficient appliances",
    "Turn off devices when not in use",
    "Avoid using heavy devices during prak hours",
    "Switch to LED lighting",
    "Use smarrt plug and scheduler",
    "Monitor real time usage through HomeWatt Dashboard"
]
corpus_embeddings=similarity_model.encode(recommendations,
                                          convert_to_tensor=True)

#Tabs for Navigation
tab1,tab2=st.tabs(["Dashboard","Ai Assistant"])

#Tab1 Creation(Dashboard)
with tab1:
    st.title("HomeWatt-Energy Analytics Dashboard")
    uploaded_file=st.file_uploader("Upload your energy usage CSV",
                                   type=["csv"])
    if uploaded_file:
        ...
        bar_chart=alt.Chart('hourly_usage').mark_bar().encode(x='hour:O',
                                                              y='usage_kwh:Q',
                                                              tooltip=['hour',
                                                              'usage_kwh']).properties(height=300)
        st.altair_chart(bar_chart,use_container_width=True)

        #Chart 3: Heatmap for day vs Hour
        st.subheader("/ Hour vs Day Usage Heatmap")
        heatmap_data=df.pivot_table(index='day',columns='hour',
                                    values='usage_kwh',
                                    aggfunc='sum').filna(0)
        sns.heatmap(heatmap_data,cmap="YIOrRd",
        ...
        st.line_chart(df_sorted.set_index('timestamp')
                      [['usage_kwh','rolling_avg']])
        ...
        st.datframes(anomalies[anomalies['anomaly']==-1])
...
with tab2:
    ...
    user_input=st.text_input("Ask HomeWatt anything  about our energy usage")
    ...
    and kitchen appliances 800W.
    Major consumption is observed between 6PM and 10PM. Usage can be optimized 
    by turning off idle devices.
    Shifting to LED lighting and scheduling heavy appliances usage during 
    non-peak hours.
    """
    answer=qa_pipeline(question=user_input,context=energy_context)
    query_emb=similaritty_model.encode(user_input,convert_to_tensor=True)
